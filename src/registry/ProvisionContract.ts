...
const getInstance = async (chainId: ChainId, signerOrProvider: ethers.providers.Provider | ethers.Signer | undefined): Promise<ethers.Contract> => {
  const key = encodeKeys(['bytes32', 'bytes32'], [NS_KEYS.CONTRACTS, NS_KEYS.COVER_PROVISION])
  const provision = await getOrFetch(chainId, key, signerOrProvider)

  return getContract(chainId, provision, abis.ICoverProvision, signerOrProvider)
}
